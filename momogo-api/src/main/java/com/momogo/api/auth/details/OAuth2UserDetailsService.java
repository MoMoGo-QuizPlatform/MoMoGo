...
public class OAuth2UserDetailsService extends DefaultOAuth2UserService {
    ...
    private User validateSocialUser(User user, SocialType socialType) {
        if (Boolean.TRUE.equals(user.getIsBanned())) {
            log.error("[OAuth2UserDetailsService] 밴 처리된 유저({}) 로그인 시도", EmailMasker.mask(user.getEmail()));
            throw new OAuth2AuthenticationException(
        ...
        if (user.getDeletedAt() != null) {
            if (!user.isRestorable()) {
                log.info("[OAuth2UserDetailsService] 복구 기간이 만료된 소셜 유저({}) 로그인 시도", EmailMasker.mask(user.getEmail()));
                throw new OAuth2AuthenticationException(
                        new OAuth2Error("restore_expired"),
                        "탈퇴 후 30일이 경과하여 복구할 수 없습니다."
                );
            }
            log.info("[OAuth2UserDetailsService] 탈퇴 대기 중인 소셜 유저({}) 복구 및 로그인 진행", EmailMasker.mask(user.getEmail()));
            user.restore();
        ...
        if (user.getSocial() != socialType) {
            log.error("[OAuth2UserDetailsService] 이메일({})은 이미 {} 방식으로 가입되어 있어 {} 로그인하실 수 없습니다.",
                    EmailMasker.mask(user.getEmail()), user.getSocial(), socialType);
            throw new OAuth2AuthenticationException(
